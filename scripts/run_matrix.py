...
def baseline_unit(out: Path, train: Path, run_name: str, *, seed: int | None = None) -> Unit:
    seed_args = ["--seed", str(seed)] if seed is not None else []
    return Unit(
        run_name, out / "report.json",
        [["wlm", "baseline", "--config", CFG_A, "--train", str(train), "--split-path",
...
def main() -> int:
    ...
    ap.add_argument("--only", nargs="*", default=["floor", "rq1", "rq2", "rq3", "seeds"],
                    choices=["floor", "rq1", "rq2", "rq3", "seeds"])
