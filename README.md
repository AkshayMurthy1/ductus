...
| 5 | **It replicates across base models *and* authors; locations move, the shape doesn't.** Three model families show the same cliff-and-plateau with zero adapter leakage. On informal-register Mark Twain the adapter climbs from a 0.47 base-model floor to 0.904 — above real Twain's 0.852 — with zero copying, on the author the base model knows *least* (contamination ratio 1.17). | 2 authors · 3 models, adapter leak **0.0%** everywhere |
